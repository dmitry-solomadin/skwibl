...
(function() {
  ...
  $(function() {
    ...
    RoomSocketHelper = (function(_super) {

      __extends(RoomSocketHelper, _super);
      ...
      function RoomSocketHelper() {
        ...
        socket.on('eraseCanvas', function() {
          _this.room().canvas.erase();
          return _this.room().redrawWithThumb();
        });
        socket.on('nextId', function() {
          return _this.opts().nextId = _this.opts().nextId + 1;
        });
      }

      RoomSocketHelper.prototype.addOrUpdateCommentText = function(data) {
        var foundComment;
        foundComment = this.room().helper.findByElementId(data.elementId);
        return this.room().comments.addCommentText(foundComment.commentMin, data.text, false);
      };
      ...
      RoomSocketHelper.prototype.addOrUpdateComment = function(data, initial) {
        ...
        updateComment = function(comment, updatedComment) {
          ...
          return _this.room().comments.redrawArrow(comment.commentMin);
        };
        createNewComment = function(comment) {
          ...
          if (comment.rect) {
            ...
            _this.room().items.create(rect, _this.room().comments.COMMENT_STYLE);
          }
          commentMin = _this.room().comments.create(comment.min.x, comment.min.y, rect, comment.max);
          commentMin.elementId = comment.elementId;
          if (rect) {
            rect.commentMin = commentMin;
            rect.eligible = false;
            return _this.room().history.add(rect);
          } else {
            return _this.room().history.add({
              type: "comment",
      ...
      RoomSocketHelper.prototype.socketRemoveElement = function(data) {
        this.room().helper.findByElementId(data).remove();
        this.room().items.unselectIfSelected(data);
        return this.room().redrawWithThumb();
      };

      RoomSocketHelper.prototype.socketRemoveComment = function(data) {
        var commentMin, element;
        element = this.room().helper.findByElementId(data);
      ...
      RoomSocketHelper.prototype.addOrUpdateElement = function(data, initial) {
        ...
        if (initial) {
          ...
          foundPath = this.room().helper.findByElementId(data.elementId);
          if (foundPath) {
            this.room().items.unselectIfSelected(foundPath.elementId);
            ...
            if (foundPath.commentMin) {
              this.room().comments.redrawArrow(foundPath.commentMin);
            }
          } else {
