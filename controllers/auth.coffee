...
exports.connectGoogle = (req, res, next) ->
  return res.redirect "https://accounts.google.com/o/oauth2/auth?client_id=#{cfg.GOOGLE_CLIENT_ID}&response_type=code&redirect_uri=http%3A%2F%2F#{cfg.HOST}/connect/google/callback&scope=https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.email+https%3A%2F%2Fwww.googleapis.com%2Fauth%2Fuserinfo.profile" #&access_type=offline'
...
exports.connectFacebook = (req, res, next) ->
  return res.redirect "https://graph.facebook.com/oauth/authorize?client_id=#{cfg.FACEBOOK_APP_ID}&redirect_uri=http%3A%2F%2F#{cfg.HOST}/connect/facebook/callback&scope=email,user_online_presence"
...
exports.connectFacebookCb = (req, res) ->
  code = req.query['code']
  tokenURL = "https://graph.facebook.com/oauth/access_token?client_id=#{cfg.FACEBOOK_APP_ID}&redirect_uri=http%3A%2F%2F#{cfg.HOST}/connect/facebook/callback&client_secret=#{cfg.FACEBOOK_APP_SECRET}&code=#{code}"
  ...
  return request.post url: url, oauth: oauth, (error, response, body) ->
    if not error and response.statusCode is 200
      ...
      return db.auth.setConnection req.user.id, 'dropbox',
        oauth_token_secret: ans.oauth_token_secret
      , (err, val) ->
        return res.redirect "https://www.dropbox.com/1/oauth/authorize?oauth_token=#{ans.oauth_token}&oauth_callback=#{cfg.DOMAIN}/connect/dropbox/callback"
